		...
		if(rawFile.readyState === rawFile.DONE && (rawFile.status === 200 || rawFile.status == 0))
		{
			...
			for (var i = 0; i < max; i++) { 
				...
				if (secondSplit.length == 13){
					//add to database.
					database = database.concat([new Herb(secondSplit[0].trim(),secondSplit[1].trim(),secondSplit[2].trim(),secondSplit[3].trim(),secondSplit[4].trim(),secondSplit[5].trim(),secondSplit[6].trim(),secondSplit[7].trim(),secondSplit[8].trim(),secondSplit[9].trim(),secondSplit[10].trim(),secondSplit[11].trim(),secondSplit[12].trim())]);
				}
			}
		}
    }
...
function searchValue(herbName) {
	var herbString = herbName.toString();
	herbString= herbString.trim();
	
	var regExpHerb = new RegExp (herbString, 'i');
...
function searchHerbs() {
	...
	var search = document.getElementById("herbName").value;
